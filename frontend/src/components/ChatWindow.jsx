...
const ChatWindow = ({ chat, activeChat }) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const res = await axios.get(`${backendApi}/api/message/${activeChat._id}`, {
    ...
    const handleAIResponse = (newMessage) => {
      if (newMessage.chatId === activeChat?._id) {
        ...
        const typingInterval = setInterval(() => {
          setTypedMessage((prev) => prev + text.charAt(index));
          index++;
          if (index >= text.length) {
            clearInterval(typingInterval);
            setIsTyping(false);
            // Add full AI message to messages list
            setMessages((prev) => [
              ...prev,
          ...
          <>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="max-w-[75%] px-4 py-2 rounded-2xl text-sm bg-[#1a2335] text-gray-200 rounded-bl-none whitespace-pre-line">
                  {typedMessage}
